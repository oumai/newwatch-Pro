...
@interface KMDeviceSettingCheckTiemVC ()
...
/** post模型 */
@property (nonatomic, strong) KMDeviceSettingCheckTimeModel *postModel;
...
@implementation KMDeviceSettingCheckTiemVC

//  postModel
- (KMDeviceSettingCheckTimeModel *)postModel
{
    if (_postModel == nil) {
        
        _postModel = [[KMDeviceSettingCheckTimeModel alloc] init];
    }
    return _postModel;
}
...
- (void)saveUserOperation
{
    ...
    // 创建请求连接
    NSString * URL = [NSString stringWithFormat:@"deviceManager/lbs/%@/%@",member.loginAccount,self.imei];
