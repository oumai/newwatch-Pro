...
/** post模型 */
@property (nonatomic, strong) KMBloodSugarModel *postModel;
...
@implementation KMBloodSugarRangeVC
...
- (void)configViewData
{
    self.upperSlider.value = _model.beforeMealH;
    self.lowerSlider.value = _model.beforeMealL;
    self.upperValue.text = [NSString stringWithFormat:@"%.1f %@",_model.beforeMealH/18.0,kLoadStringWithKey(@"DeviceManager_HealthSetting_mole")];
    self.lowerValue.text = [NSString stringWithFormat:@"%.1f %@",_model.beforeMealL/18.0,kLoadStringWithKey(@"DeviceManager_HealthSetting_mole")];
}
...
- (void)saveUserOperation
{
    ...
    self.postModel.beforeMealL  = self.lowerSlider.value;
    self.postModel.beforeMealH  = self.upperSlider.value;
    
    
    // 创建请求连接
    NSString * URL = [NSString stringWithFormat:@"healthRange/bs/%@/%@",member.loginAccount,self.imei];
...
{
    NSInteger high = self.upperSlider.value*10/18;
    NSInteger low  = self.lowerSlider.value*10/18;
...
{
    self.upperValue.text = [NSString stringWithFormat:@"%.1f %@",sender.value/18.0,kLoadStringWithKey(@"DeviceManager_HealthSetting_mole")];
}
...
{
    self.lowerValue.text = [NSString stringWithFormat:@"%.1f %@",sender.value/18.0,kLoadStringWithKey(@"DeviceManager_HealthSetting_mole")];
}
