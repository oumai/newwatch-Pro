...
- (void)touchKeyPoint:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    for (NSInteger p = _pathPoints.count - 1; p >= 0; p--) {
        ...
        for (int i = 0; i < (int)linePointsArray.count - 1; i += 1) {
            ...
            if (distance <= 20.0) {
                if ([_delegate respondsToSelector:@selector(userClickedOnChart:lineIndex:pointIndex:)]) {
                    [_delegate userClickedOnChart:self
                                        lineIndex:p
...
- (void)drawDottedLine {
    CGFloat innerGrade;
    for (int i = 0; i < self.dottedLines.count; i++) {
        CGFloat y = [self.dottedLines[i] floatValue];
        
        if (!(_yValueMax - _yValueMin)) {
            innerGrade = 0.5;
        } else {
            innerGrade = (y - _yValueMin) / (_yValueMax - _yValueMin);
        }
        
        NSInteger count = _xLabels.count > 1 ? (_xLabels.count - 1) : 0;

        int w = count *  _xLabelWidth + _chartMarginLeft + _xLabelWidth /2.0;
        int realY = _chartCavanHeight - (innerGrade * _chartCavanHeight) + (_yLabelHeight / 2)  + _chartMarginTop - _chartMarginBottom;
        
        UIView *dash = [[UIView alloc] initWithFrame:CGRectMake(_chartMarginBottom + 15, realY, w, 1)];
        [self insertSubview:dash atIndex:0];
        [self drawDashLine:dash lineLength:4 lineSpacing:4 lineColor:[UIColor redColor]];
    }
}
...
- (void)calculateChartPath:(NSMutableArray *)chartPath andPointsPath:(NSMutableArray *)pointsPath andPathKeyPoints:(NSMutableArray *)pathPoints andPathStartEndPoints:(NSMutableArray *)pointsOfPath
{
    ...
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        ...
        for (NSUInteger i = 0; i < chartData.itemCount; i++) {
            ...
            yValue = item.y;
            UIColor *pointColor = item.pointColor;
            ...
            if (chartData.inflexionPointStyle == PNLineChartPointStyleCircle) {
                ...
                // 添加一个带颜色的点
                if (pointColor) {
                    UIView *ciclePoint = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 10, 10)];
                    ciclePoint.layer.cornerRadius = 5;
                    ciclePoint.clipsToBounds = YES;
                    ciclePoint.backgroundColor = pointColor;
                    ciclePoint.backgroundColor = pointColor;
                    [self addSubview:ciclePoint];
                    ciclePoint.center = circleCenter;
                }
